...
import { assertBackupSize, parseTodoBackup } from "../backup.js";
...
function normalizeAssignee(value) {
    ...
    if (assignee.length > 100)
        throw new PersonalTodoError('assignee must contain at most 100 characters');
...
export class TodoStore {
    ...
    requireRunStatus(row, status) {
        ...
        const run = this.database.prepare('SELECT status FROM todo_runs WHERE id = ?').get(row.active_run_id ?? '');
        if (run?.status !== status)
            throw new PersonalTodoError(`待办 ${row.id} 的 Agent 执行状态必须为 ${status}。`);
        return row.active_run_id;
    }
    ...
    importData(json) {
        ...
        try {
          ...
          `);
            ...
            if (error instanceof PersonalTodoError)
                throw error;
    ...
    update(id, patch) {
        this.assertOpen();
        const mutableKeys = ['title', 'notes', 'assignee', 'priority', 'dueAt', 'tags'];
    ...
    beginRun(id, runId, sessionId) {
        ...
        if (current.status !== 'pending' && current.status !== 'in_progress')
            throw new PersonalTodoError('只有未完成任务可以交给 Agent。');
        if (current.archived_at !== null)
            throw new PersonalTodoError('请先恢复归档任务。');
        if (current.active_run_id !== null)
            throw new PersonalTodoError('请先处理当前 Agent 执行或结果。');
        const timestamp = this.now();
    ...
    setStatus(id, status) {
        ...
        if (status === current.status)
            return this.get(id);
        const run = current.active_run_id === null ? undefined : this.database.prepare('SELECT status FROM todo_runs WHERE id = ?').get(current.active_run_id);
        if (run?.status === 'running' || run?.status === 'waiting_input')
            throw new PersonalTodoError('请先停止 Agent 执行，再修改任务状态。');
        const timestamp = this.now();
      ...
      `).run(status, status === 'completed' ? timestamp : null, timestamp, id);
            ...
            this.appendEvent(id, current.active_run_id, review ? 'review_approved' : status === 'cancelled' ? 'cancelled' : 'updated', review ? null : `用户将任务标为${labels[status]}`, timestamp);
        });
        return this.get(id);
    }
    ...
    archive(id) {
        ...
        const current = this.requireRow(id);
        if (current.archived_at !== null)
            throw new PersonalTodoError(`todo ${JSON.stringify(id)} is already archived`);
        if (current.active_run_id !== null)
            throw new PersonalTodoError('请先停止或处理 Agent 执行，再归档任务。');
        const timestamp = this.now();
        this.transaction(() => {
            this.database.prepare(`
    ...
    restore(id) {
        ...
        const current = this.requireRow(id);
        if (current.archived_at === null)
            throw new PersonalTodoError(`todo ${JSON.stringify(id)} is not archived`);
        const timestamp = this.now();
        this.transaction(() => {
            this.database.prepare(`
