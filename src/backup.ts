import { z } from 'zod'
import { TODO_EVENT_TYPES, TODO_PRIORITIES, TODO_RUN_STATUSES, TODO_STATUSES, type TodoBackup } from './types.ts'
...
const todoSchema = z.strictObject({
  ...
  tags: z.array(z.string().min(1).max(32)).max(20).refine(tags =>
    new Set(tags).size === tags.length && tags.every(tag => tag === tag.trim().toLocaleLowerCase())),
...
export function parseTodoBackup(json: string): TodoBackup {
  ...
  for (const { todo, runs, sessions, events } of backup.todos) {
    const invalid = (): never => { throw new Error(`待办 ${todo.id} 的记录归属、引用关系或执行状态无效。`) }
    ...
    if ([...runs, ...sessions, ...events].some(record => record.todoId !== todo.id)) invalid()
    ...
    if (sessions.some(session => session.role === 'primary'
      && (session.sessionId !== todo.primarySessionId || session.parentSessionId !== null))) invalid()
