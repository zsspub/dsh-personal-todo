...
const DEFAULT_STATUSES = ['in_progress', 'pending'];
const EMPTY_COUNTS = {
    pending: 0,
    inProgress: 0,
    completed: 0,
...
function requestOf(query, offset, limit) {
    return {
...
/** 返回忽略分页参数后的稳定查询键。 */
export function normalizeTodoQueryKey(input, mode) {
...
function initialQuerySnapshot(seed) {
    return {
...
function matches(todo, query) {
    if ((todo.archivedAt !== null) !== query.archived)
        return false;
    if (!query.statuses.includes(todo.status))
        return false;
    if (query.priorities !== undefined && !query.priorities.includes(todo.priority))
        return false;
    if (query.tags.some(tag => !todo.tags.includes(tag)))
        return false;
    if (query.dueBefore !== undefined && (todo.dueAt === null || todo.dueAt > query.dueBefore))
        return false;
...
export class PersonalTodoDataCenter {
    ...
    #now;
    #panelRefreshMs;
    #focusListener = () => { void this.refreshSubscribed('all'); };
    #panelVisible = false;
    #panelTimer;
    #disposed = false;
    #openPanel;
    ...
    openTodo(id) {
        if (id === undefined)
            ...
            this.canvas.focusTodo(id);
        this.#openPanel?.();
    }
    setPanelVisible(visible) {
        if (this.#disposed || visible === this.#panelVisible)
            return;
    ...
    async refreshSubscribed(mode) {
        const active = [...this.#queries.values()]
            .filter(record => record.subscribers > 0 && (mode === undefined || record.mode === mode));
        await Promise.all(active.map(record => this.#refreshQuery(record).then(() => undefined, () => undefined)));
    }
    dispose() {
        if (this.#disposed)
            return;
    ...
    async #refreshQuery(record, replace = false) {
        if (this.#disposed)
            throw new Error('个人待办数据中心已释放。');
        if (record.inflight !== undefined && !replace)
    ...
    async #fetchQuery(record, signal) {
    ...
    async #loadMore(record) {
        if (record.mode === 'all')
            return this.#refreshQuery(record);
        if (record.inflight !== undefined)
            return record.inflight;
        const current = record.store.get();
        if (!current.hasMore && current.updatedAt !== undefined)
            return current;
        const { controller, release } = this.#controller();
        record.controller = controller;
        record.store.set({ ...current, refreshing: current.ids.length > 0, loading: current.ids.length === 0, error: undefined });
        const operation = this.#remote.list(requestOf(record.request, current.ids.length, record.request.pageSize), controller.signal).then(page => {
            batch(() => {
    ...
    #ingest(todos, authoritative = true) {
        if (todos.length === 0)
            return;
        ...
        batch(() => {
            ...
            if (deletedIdsChanged)
                this.deletedIds.set(deletedIds);
        });
    }
    #setDetail(detail) {
    ...
    #writeTodo(todo) {
        ...
        if (previous !== undefined && previous.revision > todo.revision)
            return;
        batch(() => {
            ...
            if (detail !== undefined)
                this.details.setKey(todo.id, { ...detail, todo });
    ...
    #removeTodo(id) {
        ...
        batch(() => {
            ...
            if (previous !== undefined)
                this.#updateCounts(previous, undefined);
            for (const record of this.#queries.values()) {
    ...
    #reconcileQueries(previous, current) {
        for (const record of this.#queries.values()) {
            const snapshot = record.store.get();
    ...
    async #mutate(id, kind, operation, signal) {
    ...
    async #withMutation(id, kind, operation, signal) {
        if (this.isMutating(id))
            throw new Error('该待办正在处理中，请稍后再试。');
    ...
    async #revalidate() {
        ...
        await Promise.all([
            ...active.map(record => this.#refreshQuery(record, true).then(() => undefined, () => undefined)),
