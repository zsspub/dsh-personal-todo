...
export function TodoToolCard({
  ...
  const filterLabel = model.query === undefined
    ...
    : [
        ...
        model.query.statuses?.map(status => status === 'in_progress' ? t('status.inProgress') : t(`status.${status}`)).join(t('format.listSeparator')),
        model.query.priorities === undefined ? undefined : t('card.priorities', {
          priorities: model.query.priorities.map(priority => t(`priority.${priority}`)).join(t('format.listSeparator')),
        }),
        model.query.tags === undefined ? undefined : t('card.tags', { tags: model.query.tags.join(t('format.listSeparator')) }),
