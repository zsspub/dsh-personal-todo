import { i as TODO_STATUSES, n as TODO_PRIORITIES, r as TODO_RUN_STATUSES, t as TODO_EVENT_TYPES } from "./types-CXeQll3B.js";
...
const PERSONAL_TODO_SCHEMA_VERSION = 5;
...
var TodoStore = class {
	...
	initialize() {
		...
		if (row.user_version > 5) throw new PersonalTodoError(`personal todo database schema ${String(row.user_version)} is newer than supported version ${String(5)}`);
	...
	approve(id) {
		this.assertOpen();
		const current = this.requireRow(id);
		if (current.status === "completed" || current.status === "cancelled") throw new PersonalTodoError(`todo ${JSON.stringify(id)} cannot be completed while ${current.status}`);
		const timestamp = this.now();
		this.transaction(() => {
			this.database.prepare(`
      ...
      `).run(timestamp, current.active_run_id);
			this.appendEvent(id, current.active_run_id, current.status === "in_review" ? "review_approved" : "updated", current.status === "in_review" ? null : "用户标记完成", timestamp);
		});
		return this.get(id);
	}
	...
	archive(id) {
		this.assertOpen();
		if (this.requireRow(id).archived_at !== null) throw new PersonalTodoError(`todo ${JSON.stringify(id)} is already archived`);
		const timestamp = this.now();
		this.transaction(() => {
			this.database.prepare(`
