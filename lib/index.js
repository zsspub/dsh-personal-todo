...
const PERSONAL_TODO_SCHEMA_VERSION = 4;
...
var TodoStore = class {
	...
	initialize() {
		...
		if (row.user_version > 4) throw new PersonalTodoError(`personal todo database schema ${String(row.user_version)} is newer than supported version ${String(4)}`);
	...
	create(input) {
      ...
      `).run(row.id, row.title, row.notes, row.status, row.priority, row.due_at, null, null, null, null, 0, 0, timestamp, timestamp, null, null);
