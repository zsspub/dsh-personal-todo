...
var TodoStore = class {
	...
	list(input = {}) {
		...
		if (input.needsAttention === true) where.push("t.status = 'in_progress' AND t.active_run_id IN (SELECT id FROM todo_runs WHERE status IN ('waiting_input', 'submitted'))");
	...
	counts() {
		...
		counts.needsAttention = this.database.prepare("SELECT COUNT(*) AS count FROM todos WHERE archived_at IS NULL AND status = 'in_progress' AND active_run_id IN (SELECT id FROM todo_runs WHERE status IN ('waiting_input', 'submitted'))").get().count;
		return counts;
	}
...
var TodoOrchestrator = class {
	...
	async exclusive(id, operation) {
		...
		try {
			...
			if (succeeded && runId !== void 0) this.settleIdle(id, runId);
		}
	...
	settleIdle(id, runId) {
		...
		if (todo.activeRunId === runId && todo.executionStatus === "running") this.store.stopExecution(id, "Agent 已停止，尚未提交结果；可重试或由用户接手。");
	}
