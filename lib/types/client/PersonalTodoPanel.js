...
/** 打开待办面板并提示待处理事项的侧栏入口。 */
export function PersonalTodoTrigger({ wide, t, list, canvas, openCanvas, useSessions }) {
    ...
    useEffect(() => {
        ...
        const refresh = () => {
            ...
            void list({ needsAttention: true, limit: 1 }, controller.signal).then(page => { canvas.setAttentionCount(page.counts.needsAttention); }, () => undefined);
        };
        refresh();
...
export function PersonalTodoCanvas(props) {
    const { t, canvas, list, get, create, update, start, reply, approve, archive, restore, requestChanges, delete: deleteTodo, openSession, useTabInfo, exportData, importData, setStatus, stop, } = props;
    ...
    const taskAction = (todo, action, operation) => {
    ...
    if (detailTodo !== undefined) {
        ...
        if (actionable && (archived || (hasExecution && !awaitingReview)))
        ...
        if (hasExecution && (!running || archived))
    ...
    const selectDetailAction = (action) => {
        ...
        switch (action) {
            case 'manualStart':
                ...
                break;
            case 'pending':
                taskAction(detailTodo, t('action.pending'), signal => setStatus({ id: detailTodo.id, status: 'pending' }, signal));
                break;
            case 'complete':
                taskAction(detailTodo, t('action.complete'), signal => approve(detailTodo.id, signal));
                break;
            case 'stop':
                taskAction(detailTodo, t('action.takeOver'), signal => stop(detailTodo.id, signal));
                break;
            case 'edit':
            ...
            case 'copy':
                ...
                break;
            case 'archive':
                taskAction(detailTodo, t('action.archive'), signal => archive(detailTodo.id, signal));
                break;
            case 'cancel':
                taskAction(detailTodo, t('action.cancelTask'), signal => setStatus({ id: detailTodo.id, status: 'cancelled' }, signal));
                break;
            case 'delete':
