...
import { isPersonalTodoPresentationMeta, type PersonalTodoPresentationMeta } from '../presentation.ts'
...
export const personalTodoTurnDefinition: ConversationNodeDefinition<PersonalTodoTurnState> = {
  kind: 'personal-todo',
  match(event) {
  ...
  start(_context, match) {
    if (match.event.type !== 'turn/start') throw new Error('个人待办回合投影需要 turn/start 事件。')
  ...
  buildLocationData({ state }, scope, previous) {
    ...
    if (previous?.key === 'personal-todo' && previous.value.results === state.results) return previous
    return {
      kind: 'turn',
      turn: state.turn,
      key: 'personal-todo',
...
export function selectPersonalTodoTail(owner: TurnTailOwnerProps): PersonalTodoTurnResult | null {
