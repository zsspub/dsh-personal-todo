...
function normalizePresentationQuery(input) {
	...
	return {
		...
		...priorities === void 0 ? {} : { priorities },
		...tags === void 0 ? {} : { tags },
		...dueBefore === void 0 || dueBefore === "" ? {} : { dueBefore },
		...search === void 0 || search === "" ? {} : { search },
		...input.archived === void 0 ? {} : { archived: input.archived }
	};
}
function boundedTodo(todo) {
	return {
		...todo,
		notes: todo.notes === null ? null : todo.notes.slice(0, 2e3),
		latestSummary: todo.latestSummary === null ? null : todo.latestSummary.slice(0, 2e3),
		blockedReason: todo.blockedReason === null ? null : todo.blockedReason.slice(0, 1e3)
	};
}
/** 创建可随 Session 日志持久化的有界卡片快照。 */
function personalTodoPresentationMeta(input, result) {
	const todos = result.todos.slice(0, 50).map(boundedTodo);
	return {
...
const LIST_PARAMETERS = {
	...
	tags: {
		type: "array",
		items: { type: "string" },
		description: "必须同时匹配的全部标签。"
	},
	dueBefore: {
		type: "string",
		description: "只显示截止时间不晚于此 RFC 3339 时间的待办。"
	},
	search: {
		type: "string",
		description: "不区分大小写搜索标题、备注和负责人。"
	},
	limit: {
		type: "integer",
		description: "分页大小；部署最大值默认为 200。"
	},
	offset: {
		type: "integer",
		description: "从零开始的分页偏移。"
	},
	archived: {
		type: "boolean",
		description: "为 true 时只显示归档待办。"
	}
};
