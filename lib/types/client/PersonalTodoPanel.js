...
export function PersonalTodoTrigger({ wide, t, list, canvas, openCanvas, useSessions }) {
    ...
    useEffect(() => {
        ...
        const refresh = () => {
            ...
            void list({ statuses: ['pending', 'blocked', 'in_review'], limit: 1 }, controller.signal).then(page => { canvas.setAttentionCount(page.counts.pending + page.counts.blocked + page.counts.inReview); }, () => undefined);
        };
        refresh();
...
export function PersonalTodoCanvas(props) {
    const { t, canvas, list, get, create, update, start, reply, approve, archive, restore, requestChanges, delete: deleteTodo, openSession, useTabInfo, exportData, importData, } = props;
