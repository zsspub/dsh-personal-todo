...
export function codexDelegationPrompt(todo) {
...
function registerCodexDelegationTool(ctx, delegate) {
    ctx.tools.register(defineTool({
        ...
        async execute(args, exec) {
            ...
            if (exec.agent === undefined)
                throw new Error('personal todo delegation requires an Agent Session');
...
export function apply(ctx) {
    ctx.inject(['codexDelegate'], (scope) => {
        ...
        if (delegate === undefined)
            throw new Error('personal-todo-tools: codexDelegate injection resolved without a service');
        registerCodexDelegationTool(scope, delegate);
        scope.tools.guard((exec) => {
            if (exec.name !== delegate.genericToolName || exec.agent === undefined)
                return undefined;
